  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden sticky top-4 shadow-sm">
      ...
      <div className="bg-gradient-to-br from-red-600 to-red-700 p-4 text-white">
        ...
        {p.price_per_ping && (
          <p className="text-red-200 text-xs mt-1">
            {lang === "zh"
              ? `每坪 ${p.price_per_ping.toLocaleString()}萬`
              : `${p.price_per_ping.toLocaleString()} vạn/ping`}
          </p>
        )}
      </div>
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100">
        ...
        <button className="ml-auto text-[11px] text-red-600 border border-red-200 rounded-lg
                           px-2.5 py-1 hover:bg-red-50 transition shrink-0">
          {t.scheduleVisit}
        </button>
      </div>
